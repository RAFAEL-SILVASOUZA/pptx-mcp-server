...
/**
 * Loads an HTML file in a real browser (so CSS layout — flexbox, grid, fonts,
 * everything — is computed exactly like it would be on screen) and reads back
 * the rendered position/style of every element the author explicitly marked
 * with a `data-pptx` attribute. No screenshot is taken; only geometry and
 * computed style values are extracted, for mapping onto native PPTX objects.
 */
export async function extractPresentation(htmlPath: string): Promise<ExtractedSlide[]> {
  const browser = await getBrowser();
  const page = await browser.newPage();
  try {
    const fileUrl = pathToFileURL(htmlPath).href;
    await page.goto(fileUrl, { waitUntil: "networkidle" });
    return await page.evaluate(extractInBrowser);
  } finally {
    await page.close();
  }
}
 ...
 */
function extractInBrowser(): ExtractedSlide[] {
